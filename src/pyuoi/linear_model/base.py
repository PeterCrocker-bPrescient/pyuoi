...
class AbstractUoILinearModel(SparseCoefMixin, metaclass=_abc.ABCMeta):
    ...
    def fit(self, X, y, stratify=None, verbose=False):
        ...
        for boot in range(self.n_boots_sel):
            if size > 1:
                if rank == 0:
                    ...
                    rvals = [None] * 2
                rvals = [
                    Bcast_from_root(rval, self.comm, root=0) for rval in rvals
                ]
        ...
        # set up data arrays
        tasks = np.array_split(
            np.arange(self.n_boots_est * self.n_supports_), size
        )[rank]
        my_boots = dict(
            (task_idx // self.n_supports_, None) for task_idx in tasks
        )
        ...
        if size > 1:
            ...
            self.intercept_ = Bcast_from_root(
                self.intercept_, self.comm, root=0
            )
...
class AbstractUoILinearRegressor(
    ...
    def _score_predictions(self, metric, fitter, X, y, support, boot_idxs):
        ...
        if metric == "r2":
            ...
            ll = utils.log_likelihood_glm(
                model="normal", y_true=y, y_pred=y_pred
            )
    ...
    def _fit_intercept(self, X, y):
        """Fit the intercept."""
        if self.fit_intercept:
            self.intercept_ = y.mean(axis=0) - np.dot(
                X.mean(axis=0), self.coef_.T
            )
        else:
